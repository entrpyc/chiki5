...
function paintAll(c, rgbAt, alphaAt) {
  for (let y = 0; y < c.h; y++) {
    for (let x = 0; x < c.w; x++) {
...
function sdCircle(cx, cy, r) {
  return (x, y) => Math.hypot(x - cx, y - cy) - r
}

function sdRoundBox(cx, cy, hx, hy, r) {
  return (x, y) => {
...
function sdSegment(ax, ay, bx, by, r) {
  return (x, y) => {
    const ex = bx - ax
    const ey = by - ay
    const wx = x - ax
...
const mirrorX = (f, w) => (x, y) => f(w - x, y)
const shrink = (f, r) => (x, y) => f(x, y) + r

/** Blunts a polygon's corners: pull each vertex toward the centroid, then swell by the radius. */
function sdRoundPolygon(verts, r) {
